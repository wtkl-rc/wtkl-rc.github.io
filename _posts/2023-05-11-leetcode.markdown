...
public:
    ...
    void addAtIndex(int index, int val) {
        if(index>_size&&index<0){        //判断index是否合法
            exit;
        }

        LinkedList* p=_head;
        int i=0;
        while(i!=index-1&&p!=NULL&&p->next!=NULL){   //找到index-1的结点
            p=p->next;
            i++;
        }
        ...
        if(index+1==_size){
            ...
            LinkedList* nw=new LinkedList;      //新建一个结点插入
            nw->val=val;
            nw->next=p->next;
            p->next=nw; 
            _size++;
    ...
    void deleteAtIndex(int index) {
        ...
        LinkedList* back=_head;
        int i=0;
        while(i!=index-1&& back!=NULL&& back->next!=NULL){   //找到index-1的结点
            back=back->next;
            i++;
        }
        LinkedList* middle=back->next;
